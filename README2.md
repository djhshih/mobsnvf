...
| `--damage-type`| `-d`, `--dt` | No | `ffpe` | The type of DNA damage to model. Supported values are `ffpe` and `oxog`. |
| `--out-dir` | `-o` | No | `./result` | The base directory where results will be saved. |
...
If you are sure that yout BAM uses a standard HG38 reference genome, you can use the `get.sh` script included in the ref directory to download it:
...
**Important:** The example reference genome files need to be downloaded. Please navigate to the `example-data/ref` directory and run the following command to download the reference genome using the included makefile:
...
bash mobsnvf-workflow.sh \
    ...
    --ref-path "/path/to/your/GRCh38.d1.vd1.fa" \
```
...
1.  **Create a manifest file**
    ...
    **Example `samples.tsv`:**
    ...
    ```tsv
    sample01	/path/to/bams/sample01.bam	/path/to/vcfs/sample01.vcf.gz
    sample02	/path/to/bams/sample02.bam	/path/to/vcfs/sample02.vcf.gz
    sample03	/path/to/bams/sample03.bam	/path/to/vcfs/sample03.vcf.gz
    ```

2.  **Run the workflow using a loop**

    You can now use a simple `bash` loop to read the manifest file and execute the workflow for each sample. This can be integrated with a job scheduler like **Dlazy** by piping the generated commands to it.

    ```bash
    #!/bin/bash
    
    REF_GENOME="/path/to/your/hg38.fasta"
    ...
    while IFS=$'\t' read -r sampleid bampath vcfpath; do
      # Skip header or empty lines
      [[ "$sampleid" == "sampleid" || -z "$sampleid" ]] && continue
      ...
      echo "bash mobsnvf-workflow.sh \
        ...
        --ref-path \"${REF_GENOME}\" \
        --out-dir \"${OUTPUT_DIR}\""
    done < samples.tsv | tee commands_to_run.sh
    
    # Optional: Pipe to a job scheduler like Dlazy
    # cat commands_to_run.sh | dlazy job
    ```